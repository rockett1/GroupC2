...
class Object:
	...
	def get_colour(self):
		...
		return(self.colour)
	def get_strColour(self):
		return(self.stringColour())
class Triangle(Object):
	...
	def print_obj_sort(self,x,y):
		return(pygame.draw.polygon(self.screen,self.colour,((x,y),(x+self.dimension,y),(x+(self.dimension/2),y+self.dimension))))
	def __str__(self):
...
class Square(Quadrilater):
	...
	def print_obj_sort(self,x,y):
		return(pygame.draw.rect(self.screen,self.colour,(x,y,self.heigth,self.weight)))
	def __str__(self):
		return(__class__.__name__ + " with a price of " + str(self.price)+" and the "+str(self.strColour)+" colour.")
	def get_shape(self):
